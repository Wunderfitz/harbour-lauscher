...
namespace mdr
{
    ...
    MDRTask MDRHeadphones::RequestInitV1()
    {
        ...
        /* Battery level, one request per part the device advertises: a single battery for
         * headphones, left and right for earbuds, and the charging case besides. */
        ...
        if (state.mSupport.contains(t1::FunctionType::LEFT_RIGHT_BATTERY_LEVEL))
        {
            SendCommandACK(t1::GetBatteryLevel,
                           {.batteryInquiredType = t1::BatteryInquiredType::LEFT_RIGHT_BATTERY});
        }
        if (state.mSupport.contains(t1::FunctionType::CRADLE_BATTERY_LEVEL))
        {
            SendCommandACK(t1::GetBatteryLevel,
                           {.batteryInquiredType = t1::BatteryInquiredType::CRADLE_BATTERY});
        }
